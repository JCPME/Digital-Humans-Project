...
def process_hamer(nancount, seq_path, save_joints=False):
    ...
    results = altbase(results)

    #results = improve(results,1/30,4e3,8e-2) #q-change in velocity r-detection error #2000 0.001

    #results = hand_correction2(results)
    
    #results = hand_selection(results)
    #results = z_corr1(results,10,1)
    #results = hand_fill2(results)
    #results = transl_filter(results,30)
    #results = smooth(results,1/30,0.0001,0.01)
    ...
    pred_hand_joints = butter_lowpass_filter(pred_hand_joints, cutoff=0.1, fs=30, order=2)
...
for result_path in tqdm(result_path_list):
    metrics,nancount = process_hamer(nancount, result_path, save_joints=True)
    print("hammering")
